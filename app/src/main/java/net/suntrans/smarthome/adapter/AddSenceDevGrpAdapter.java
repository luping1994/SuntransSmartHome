package net.suntrans.smarthome.adapter;
...
public class AddSenceDevGrpAdapter extends BaseExpandableListAdapter {
    ...
    public int getChildrenCount(int groupPosition) {
        return datas.get(groupPosition).channel.size();
    }
    ...
    public Object getChild(int groupPosition, int childPosition) {
        return datas.get(groupPosition).channel.get(childPosition);
    }
    ...
    public class GroupHolder {
        ...
        public void setData(final int groupPosition) {
            mName.setText(datas.get(groupPosition).house.name);
            ...
            state.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (state.isChecked()) {
                        for (int i = 0; i < datas.get(groupPosition).channel.size(); i++) {
                            datas.get(groupPosition).channel.get(i).setChecked(true);
                        }
                        datas.get(groupPosition).setCheck(true);
                    } else {
                        for (int i = 0; i < datas.get(groupPosition).channel.size(); i++) {
                            datas.get(groupPosition).channel.get(i).setChecked(false);
                        }
            ...
            count.setText(datas.get(groupPosition).channel.size() + "");
        }
    }
    ...
    public class ChildHolder {
        ...
        public void setData(final int groupPosition, final int childPosition) {
            mText.setText(datas.get(groupPosition).channel.get(childPosition).name);
            state.setChecked(datas.get(groupPosition).channel.get(childPosition).isChecked());
            state.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    datas.get(groupPosition).channel.get(childPosition).setChecked(state.isChecked());
                    int checkedCount = 0;
                    for (int i = 0; i < datas.get(groupPosition).channel.size(); i++) {
                        if (datas.get(groupPosition).channel.get(i).isChecked())
                            checkedCount++;
                    }
                    if (checkedCount == datas.get(groupPosition).channel.size()) {
                        datas.get(groupPosition).setCheck(true);
                    } else {
                        datas.get(groupPosition).setCheck(false);
